...
// number 0-100, streak a number 0-366 (a streak can run longer than a
// hundred days), weakestHabit at most 60 characters with control
...
// number or numeric string within [0, max] -> number, else null
function numIn(v, max) {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const body = req.body || {};
    const weeklyPercent = numIn(body.weeklyPercent, PERCENT_MAX);
    const streak = numIn(body.streak, STREAK_MAX);
    const weakestHabit = habitText(body.weakestHabit);
    if (weeklyPercent === null) return res.status(400).json({ error: 'weeklyPercent must be a number 0-' + PERCENT_MAX });
    if (streak === null)        return res.status(400).json({ error: 'streak must be a number 0-' + STREAK_MAX });
